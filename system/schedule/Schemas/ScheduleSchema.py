...
class ScheduleSchema(Schema):
    ...
    day = fields.List(fields.Int(
        required=True, validate=(validate.Range(0, 6))))
    specific_week = fields.List(fields.Int(
        allow_none=True, validate=(validate.Range(1, 4))))
    slot_start = fields.Time(required=True)
    slot_end = fields.Time(allow_none=True)
    booking_start = fields.Int()
    booking_end = fields.Int()
    fees = fields.Int()
    patient_limit = fields.Int(allow_none=True)
    ...
    def check(self, data, **kwargs):
        ...
        if not data.get("clinic_name") and not data.get("medical_shop"):
            raise ValidationError(
                'Atleast one of medical shop and clinic name should be present', Config.RESPONSE_KEY)
